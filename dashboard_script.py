...
def get_connectivity_journeys_matrix(search_value):
    ...
    e={'TopTop' : 1, 'TopMiddle' : 2, 'TopBottom': 3, 'MiddleTop':1, 'MiddleMiddle':2, 'MiddleBottom':3, 'BottomTop':2, 'BottomMiddle':3, 'BottomBottom':5 }
...
def input_OD_Matrix():
    ...
    if reading_from_access:
        ...
        vs1 = pd.read_csv(r'C:\Users\Kharesa-Kesa.Spencer\OneDrive - Arup\Projects\Network Rail Accessibility case\matrices\Vector save.csv')
        vs2 = pd.read_csv(r'C:\Users\Kharesa-Kesa.Spencer\OneDrive - Arup\Projects\Network Rail Accessibility case\matrices\Vector save 2.csv')
        frames = [vs1,vs2]
...
def get_new_categories_set_jrnys(base_df):
    ...
    for index, row in base_df.iterrows():
        ...
        if row['Inaccessible_(1_if_not_Step_Free_Cat._A_or_B1)'] == 1:
            ...
            unlock_jrny.append(int(row['2019_Journeys_to_an_accessible_destination'] + row['2019_Journeys_from_an_accessible_origin']))
    ...
    for index, row in base_df.iterrows():
        ...
        if mob == 'nan':
            ...
            base_df.loc[base_df['Connectivity_and_Journeys_Matrix_Outcome']==mob, 'Connectivity_and_Journeys_Matrix_Outcome.1'] = get_connectivity_journeys_matrix(mob)
...
def set_mobility_isolation_score(updated_cats_and_jrnys, alt_any_df):
    ...
    for target in target_stns:
        ...
        if target in updated_cats_and_jrnys.values:
            ...
            updated_cats_and_jrnys.loc[(updated_cats_and_jrnys.Unique_Code == target) & (updated_cats_and_jrnys['Inaccessible_(1_if_not_Step_Free_Cat._A_or_B1)']==1), 'Revisited_Isolation_score'] = 'Bottom'
